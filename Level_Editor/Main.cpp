...
{
	//setup the window
	MainMenuWindow.window.create(sf::VideoMode(MainMenuWindow.windowWidth, MainMenuWindow.windowHeight), "Main Menu", sf::Style::Titlebar | sf::Style::Close);
...
int MainMenu::Update()
{
	while (MainMenuWindow.window.isOpen())
	{
		sf::Event event;
		while (MainMenuWindow.window.pollEvent(event))
		{
			switch (event.type) {
			case sf::Event::Closed:
				MainMenuWindow.window.close();
				break;
			}
		}
		MainMenuWindow.window.clear(sf::Color::White);
		MainMenuWindow.window.draw(levelEditorButton);
		MainMenuWindow.window.draw(gameButton);
		MainMenuWindow.window.display();
	}
	return 0;
}
