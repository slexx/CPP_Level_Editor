	...
	void save(Tile** incTile)
	{
		...
		if (myfile.is_open())
		{
			...
			if (!enemyPos.empty())
			{
				...
				myfile << "Enemy";
				std::list < sf::Vector2i >::iterator cIt;
				for (cIt = enemyPos.begin(); cIt != enemyPos.end(); cIt++)
				{
					sf::Vector2i curEnemyPos = sf::Vector2i(cIt->x, cIt->y);
			...
			if (!spikePos.empty())
			{
				...
				myfile << "Enemy";
				std::list < sf::Vector2i >::iterator cIt;
				for (cIt = spikePos.begin(); cIt != spikePos.end(); cIt++)
				{
					sf::Vector2i curSpikePos = sf::Vector2i(cIt->x, cIt->y);
					std::cout << "Saving enemy at: " << curSpikePos.x << "," << curSpikePos.y << "\n";
			...
			myfile << "Player" << '(' << exitPos.x << ',' << exitPos.y << ')' << "\n";
